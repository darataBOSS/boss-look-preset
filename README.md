...
- **シーン整理**: 生成物 (プローブ / リフレクション / ライトリグ / Volume / グラウンドシャドウ) は空の親 `BOSS Look [<名前>]` 配下に自動でまとまります (ワールド座標は維持)。旧シーンはヘッダーの「🗂 まとめる」ボタンで整理可能。

- **★ルック** — ワンクリックのルックプリセット集 (シネマティック / 明るい物販 / ムーディ / ビビッド)。グレード+Bloom/Vignette+フォグ+ライト比を一括設定し、強さスライダでブレンド。カスタムルックの保存・適用も可能。**出力ターゲット**で「汎用 (Linear)」と「STYLY モバイル (Gamma)」を切り替え可能で、STYLY モバイルでは Linear にできない制約下でもルックが効くよう、ライティング焼き込み + LDR ポストの2層で反映
- **環境光 / ベイク** — HDRI スカイボックス (回転 / 露出 / 太陽整列) → ライトプローブ / リフレクション (解像度指定) → ベイク → AR 化
- **ライト** — 3タイプ切替: 3点照明 (被写体フォーカス) / 太陽光 (屋外、クイックプリセット付き) / シーリンググリッド (広い室内、全灯Baked)
- **ポスト** — Built-in: PPv2 / URP: Volume (自動判定)。露出・コントラスト・彩度・色温度などのカラーグレードをライブ調整
- **仕上げ** — ARグラウンドシャドウ (シャドウキャッチャー) + 距離フォグ
- **品質** — アンチエイリアス (FXAA/SMAA/MSAA) + 影品質 (距離 / カスケード / 解像度 / ソフトシャドウ)
- **診断** — Gamma色空間・純白アルベド・UV2欠落などを一括検出し、選択 / 自動修正
