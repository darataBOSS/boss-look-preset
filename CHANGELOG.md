...
- ツールが生成するシーンオブジェクト (ライトプローブ / リフレクションプローブ /
  ライトリグ / Post Process Volume / URP Volume / グラウンドシャドウ) を、
  プリセットごとの空の親 **`BOSS Look [<名前>]`** の配下に自動でまとめるように。
  ヒエラルキーが散らからず、後から触りやすくなります。
- 生成・更新時に自動で集約。ワールド座標は保持するので見た目は変わりません。
- ヘッダーに **「🗂 生成物を親オブジェクトにまとめる」** ボタンを追加。旧シーン
  (集約前に作ったもの) もワンクリックで整理できます。
- Post Process Layer はカメラ上のコンポーネントなので親変更の対象外。
